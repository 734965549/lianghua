...
def get_report(
    ...
    if row is None:
        raise BizError(ErrorCode.AI_REPORT_NOT_FOUND, f"报告不存在: {report_id}", status=404)
...
def generate_report(
    ...
    if body.range_end < body.range_start:
        raise BizError(ErrorCode.AI_REPORT_INVALID_RANGE, "结束时间不能早于开始时间")
    svc = AiReportService(db, correlation_id=correlation_id)
    try:
        result = svc.generate(
        ...
        db.rollback()
        raise BizError(ErrorCode.AI_REPORT_FAILED, f"报告生成失败: {exc}", retryable=True) from exc
